...
<div class="animated fadeIn">
    <div class="card">
        <div class="card-header">
            <h3 class="text-center"><b>NOTIS PINDAAN TARIKH MESYUARAT</b></h3>
        </div>

        <div class="card-body">

            <form class="form" action="{{ route('m_PindaanTarikh')}}">
            ...
            <body>
                ...
                <table id="bootstrap-data-table-export" class="table table-striped table-bordered">
                    ...
                    <tr>
                        ...
                        <td align="center"><strong>{{ date('d/m/Y', strtotime($event->start)) }} </strong><br>

                        <td>
                            <strong>{{ $event->TajukMesyuarat->nama_tajuk }} ({{ $event->title }})
                            <br>Bil. {{ $event->meeting_numbers }}/{{ $event->year }}</strong>
                            @if ($event->pindaan == 'Y')
                            <br><span class="badge badge-dark">Pindaan {{ $event->pindaan_ke }}</span></strong>
                            @elseif ($event->pindaan == 'N')
                            @endif
                        </td>

                        <td align="center">
                            @if($event->title == 'KSUKP')
                            <a title="Notis Pindaan Mesyuarat {{ $event->title }}" class="btn btn-success btn-sm rounded-circle" href="{{ route('m_CetakanPindaanTarikhMesyuarat_ksukp',$event->id) }}">
                                <i class="fa fa-print"></i>
                            </a>
                            @elseif($event->title == 'MBKM')
                            <a title="Notis Pindaan Mesyuarat {{ $event->title }}" class="btn btn-success btn-sm rounded-circle" href="{{ route('m_CetakanPindaanTarikhMesyuarat_mbkm',$event->id) }}">
                                <i class="fa fa-print"></i>
                            </a>
                            @endif
                        </td>
